...
public class CollaborationEvent {
    ...
    public void assigned(CollaborationAffairsAssignedEvent event) throws BusinessException {
        ...
        //同一节点多个人
        String commonNodeSql = "select id,member_id,process_id,activity_id from CTP_AFFAIR where object_id= " + currentAffair.getObjectId() + " and activity_id= " + currentAffair.getActivityId();
        boolean isJz = false;
        if (list.size() > 0) {
            //todo
            String processSql = "select process_xml from WF_PROCESS_RUNNING where id =" + currentAffair.getProcessId();
            ...
            String activityId = currentAffair.getActivityId().longValue() + "";
